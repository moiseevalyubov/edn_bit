...
6. При ошибке — автоматический retry (до 6 раз); если доставка окончательно не удалась, оператору в тот же диалог Открытой линии вставляется уведомление «не доставлено» (у Bitrix нет нативного статуса недоставки для исходящих — это обходной путь через `imconnector.send.messages`)
...
### Два типа канала: MAX Bot и MAX
Поддерживаются оба типа канала edna. Тип определяется сам при подключении (из `/channel-profile`) и хранится в `channels.channel_type`. Отличия — только в отправке: MAX Bot идёт на `/v1/out-messages/max-bot` с `sender`/`maxId`, MAX — на `/v1/out-messages/max` с `from`/`to:{value,type}`. Подробности в [docs/PRD.md](docs/PRD.md), раздел 4.3.2.

### Маршрутизация канала
Один портал Bitrix может иметь несколько каналов. При исходящем сообщении нужно понять, через какой отправлять — Bitrix присылает только `chat.id`:
1. **Помеченный `chat.id`** вида `{sender}:{identifier}` (все типы канала, кроме MAX Bot) — канал определяется однозначно по `sender`. Метка нужна потому, что один человек имеет одинаковый идентификатор в MAX и MAX Bot, и без неё диалоги слились бы в один
2. **Голый `chat.id`** (MAX Bot и каналы без сохранённого типа) — ищем активный канал, через который ранее приходили сообщения от этого `chat_id` (по таблице `messages`)
3. Если не найден — берём первый активный канал портала (fallback)

В edna и в базу всегда уходит «голый» идентификатор; метка живёт только на стороне Bitrix.
...
|---|---|---|
| `APP_BASE_URL` | **Да** | Публичный HTTPS-URL сервера, напр. `https://ednabitrix-moiseevalyubov.amvera.io`. Используется для вебхуков и файлового кеша. Без неё события Bitrix работать не будут. |
...
Отладочный стенд живёт на [Amvera](https://amvera.ru), регион Moscow-0: `https://ednabitrix-moiseevalyubov.amvera.io`. Сборка идёт по `Dockerfile`, при push в `master` деплой запускается автоматически. Переменные окружения задаются на вкладке «Переменные», порт контейнера — 8000 на вкладке «Конфигурация» (по умолчанию Amvera ждёт 80, при несовпадении отдаёт 503 при зелёном статусе).

Прод — отдельный контур внутри edna, обновляется девопсом.

Переехали с Render 2026-08-12: он стоит за Cloudflare, и связь Россия ↔ Render рвалась — edna не могла ни достучаться до webhook, ни скачать файл. Подробности и результаты проверок — в [review.md](review.md), раздел «Переезд отладочного стенда с Render на Amvera».
