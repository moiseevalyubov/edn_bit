...
7. Дальнейшую судьбу сообщения edna сообщает вебхуком на `POST /status/{webhook_token}` — см. ниже

### Статусы доставки (`/status/{webhook_token}`)

edna присылает статус отправленного сообщения; сопоставляем по `requestId`, который равен `outMessageId` из ответа на отправку (он сохраняется у сообщения в `messages.edna_request_id`).

- **`READ`** → под сообщением оператора появляется «Просмотрено» (`imconnector.send.status.delivery`). Это единственная метка, которую дают Открытые линии: обработав «доставлено», Битрикс сразу помечает сообщение прочитанным, так что выбор — «Просмотрено» или ничего.
- **`SENT`** → ничего. Значит лишь «edna передала сообщение в MAX», клиент его ещё не открывал.
- **`UNDELIVERED` / `FAILED` / `CANCELLED` / `EXPIRED`** → оператору уходит уведомление о недоставке (то же, что в п. 6). Ловит случай, когда наш запрос прошёл, а клиенту сообщение так и не доставили.

Тип канала в коде при этом не проверяется: реагируем на факт прочтения. В канале **MAX** `READ` приходит секунд через 15 после того, как клиент открыл сообщение; в **MAX Bot** его не бывает вовсе, поэтому там метки нет — и это не особый случай в коде, а следствие того, что сигнала не существует.
