...
| `identifiers` | array | Все известные edna идентификаторы отправителя, каждый со своим типом |     |

### `identifiers` — типы идентификаторов

Присутствует и у канала MAX, и у MAX Bot. Каждый элемент — `{"type": "MAX_ID"|"PHONE", "value": "..."}`.

```json
"subscriber": {
    "id": 182213104,
    "identifier": "390309887",
    "identifiers": [{"type": "MAX_ID", "value": "390309887"}]
}
```

Важное, проверенное на живых клиентах (2026-07-29):

- **`MAX_ID` есть всегда и всегда совпадает с `identifier`.** У одного и того же человека `MAX_ID` **одинаков** в каналах MAX и MAX Bot — по нему нельзя различить канал.
- **`PHONE` необязателен.** Он появляется, только если edna смогла его получить (например, склеила профиль абонента между каналами). У нового клиента массив может содержать один лишь `MAX_ID`.
- Тип нужного идентификатора определяется по элементу, чей `value` совпадает с `subscriber.identifier`. Не нашли — считаем `MAX_ID`.

### Отличие канала MAX

У канала **MAX** поле `userInfo` всегда приходит как `null` — имени клиента в вебхуке нет. У MAX Bot оно заполнено.
