...
| Подпись к файлу | **`text`** (в манифесте `caption`, но он не работает) | **`text`** |
| Имя файла | `name` (обязательно) | поля нет |

Две ловушки, обе стоили времени:

1. **`caption` бесполезно слать в оба канала** — edna не ругается, а молча выбрасывает поле, и текст оператора теряется. Подпись везде называется `text`. Для MAX выяснено 2026-08-05, для MAX Bot — 2026-08-11 (три пробы в Postman: с `caption` текст пропадал, с `text` дошёл), хотя манифест edna для MAX Bot документирует именно `caption`.
...
При ошибке оператору пишется служебная строка в чат (задача #2). Метода
`imconnector.send.status.undelivered` в Bitrix24 **не существует** — в списке
методов коннектора статусный ровно один, `imconnector.send.status.delivery`.

---

## Статусы доставки (вебхук `statusCallbackUrl`)

Адрес прописывается методом `POST /api/callback/set` — **отдельным запросом** от
входящих: метод обновляет только переданные поля, а edna при этом подписывает
вебхук именем в личном кабинете. Поле называется `statusCallbackUrl`
(в актуальной документации edna — массив `statusCallbackUrls`, но работает и
единственное число, как и `inMessageCallbackUrl`).

Тело вебхука:

```json
{
  "requestId": "019ff549-9d4f-7969-9544-9f841e68fab4",
  "messageId": 968419307,
  "subject": "79623693343_max_lm",
  "subjectId": 14043,
  "status": "READ",
  "statusAt": "2026-08-12T09:26:05.262473618Z"
}
```

**Сопоставление — по `requestId`:** он равен `outMessageId` из ответа на отправку.
Собственный `messageId` edna к нашим данным не привязан.

Что реально приходит (проверено на живых каналах 2026-08-12):
...
`DELIVERED` не приходит ни в одном канале, хотя в документации значится.

edna ждёт HTTP 200 и при неудаче повторяет всего три раза — через 4 с, 128 с и
2048 с, поэтому отвечать ошибкой почти бессмысленно.
